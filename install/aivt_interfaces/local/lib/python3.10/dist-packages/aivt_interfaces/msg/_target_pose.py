...
class Metaclass_TargetPose(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            module = import_type_support('aivt_interfaces')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'aivt_interfaces.msg.TargetPose')
            logger.debug(
...
class TargetPose(metaclass=Metaclass_TargetPose):
    ...
    def __init__(self, **kwargs):
        ...
        if 'target_pose' not in kwargs:
            self.target_pose = numpy.zeros(6, dtype=numpy.float32)
        else:
            self.target_pose = numpy.array(kwargs.get('target_pose'), dtype=numpy.float32)
            assert self.target_pose.shape == (6, )
    ...
    def target_pose(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.float32, \
                "The 'target_pose' numpy.ndarray() must have the dtype of 'numpy.float32'"
            assert value.size == 6, \
        ...
        if __debug__:
            ...
            assert \
                ...
                "The 'target_pose' field must be a set or sequence with length 6 and each value of type 'float' and each float in [-340282346600000016151267322115014000640.000000, 340282346600000016151267322115014000640.000000]"
        self._target_pose = numpy.array(value, dtype=numpy.float32)
